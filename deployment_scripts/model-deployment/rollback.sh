...
# Use || true to prevent exit if grep fails but command succeeds
...
rollback_image_tags() {
  ...
  if [[ -n "$LATEST_DIGEST" ]]; then
     ...
     # Use || true to allow continuation even if tagging/deletion fails
  ...
  if [[ -n "$V1_DIGEST" ]]; then
    ...
    gcloud artifacts docker tags add "${IMAGE_BASE}@${V1_DIGEST}" "${IMAGE_BASE}:latest" --quiet || \
       (echo "ERROR: Failed to tag v1 as latest for ${IMAGE_TYPE}."; CAN_ROLLBACK=false; ROLLBACK_SUCCESS=false)
    if [ "$CAN_ROLLBACK" = true ]; then
        echo "'v1' image is now also tagged as 'latest' for ${IMAGE_TYPE}."
    fi
  else
    # MODIFIED: Don't exit, just log error and prevent K8s apply for this image
...
# Capture success/failure for each image type
...
if [ "$ROLLBACK_SUCCESS" = true ]; then
    ...
    # Apply Backend Deployment if its tagging was okay
    ...
    # Apply Frontend Deployment if its tagging was okay
