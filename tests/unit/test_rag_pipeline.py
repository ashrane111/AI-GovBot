...
def test_retriever_component(mock_config):
    ...
    with patch('langchain_community.vectorstores.FAISS.load_local') as mock_faiss:
        ...
        # Act
        docs, scores = retriever.retrieve("test query")
...
def test_score_threshold_filtering(mock_config):
    # Arrange
    pipeline = RAGPipeline()
    pipeline.SCORE_THRESHOLD = 1.0
    test_docs = ["doc1", "doc2", "doc3"]
    test_scores = [0.8, 1.2, 0.9]
    
    # Act
    context = pipeline._RAGPipeline__generate_context(test_docs, test_scores)
    
    # Assert
    assert context.count("Rank") == 2  # Only scores below 1.0 should pass
